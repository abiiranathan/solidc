...
char *regex_replace_all(const char *str, const char *pattern,
                        const char *replacement)
    __attribute__((warn_unused_result));
...
#define string_foreach_rev(str, c)                                             \
  for (char *c = str + strlen(str) - 1; c >= str; c--)
...
char **regex_capture(const char *str, const char *regex, int num_capture_groups,
                     int *num_matches) __attribute__((warn_unused_result));
#endif
...
bool strings_equal(const char *a, const char *b) { return strcmp(a, b) == 0; }
bool strings_equal_nocase(const char *a, const char *b) {
...
char *string_snakecase(const char *s) {
  ...
  for (size_t i = 1; i < length; i++) {
    ...
    if (isupper(str[i])) {
      ...
      memmove(&str[i + 1 - space_count], &str[i - space_count],
...
char *string_replace(const char *str, const char *old, const char *new_str) {
  ...
  memmove(res + (p - str) + new_len, p + old_len,
          str_len - (p - str) - old_len + 1);
  memcpy(res + (p - str), new_str, new_len);
  return res;
}

char *string_replace_all(const char *str, const char *old,
                         const char *new_str) {
  size_t old_len = strlen(old);
  size_t new_len = strlen(new_str);
  size_t count = 0;
...
void string_trim_char(char *str, char c) { string_trim_chars(str, &c); }
...
char *regex_sub_match_pcre(const char *str, const char *regex,
  ...
  compiled_regex = pcre2_compile(pattern, PCRE2_ZERO_TERMINATED, 0, &error_code,
                                 &error_offset, NULL);
  ...
  int result =
      pcre2_match(compiled_regex, subject, strlen(str), 0, 0, match_data, NULL);
...
char **regex_capture(const char *str, const char *regex, int num_capture_groups,
  ...
  *num_matches = result;
  ...
  char **sub_matches =
      (char **)malloc(match_count * num_capture_groups * sizeof(char *));
  if (sub_matches == NULL) {
    printf("Memory allocation failed\n");
    pcre2_match_data_free(match_data);
    pcre2_code_free(compiled_regex);
