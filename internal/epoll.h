 ...
 *
 *        This function configures the socket to send keep-alive probes after a
 * period of inactivity to detect dead peers.
 *
 * @param sock_fd The file descriptor of the socket.
 * @return true on success, false on failure.
 */
...
/**
 * @brief Modifies the events being monitored for a file descriptor in the epoll
 * instance.
 *
 * @param epoll_fd The file descriptor of the epoll instance.
 ...
 * @param ipstr     A buffer to store the IP address string (must be at least
...
/**
 * @brief Creates a socket, binds it to the specified port, and sets it to
 * listen for connections.
 *
 * @param port      The port number to bind the socket to.
 * @param reuseport A flag indicating whether to allow reuse of the port.
 * @return The file descriptor of the listening socket on success, -1 on
 * failure.
 */
int epoll_create_and_bind_socket(const char* port, bool reuseport);

/**
 * @brief Sends all data in a buffer to a socket, handling partial sends and
 * retries.
 *
 ...
 * @return The number of bytes read on success, 0 on connection close, -1 on
 * error.
 */
ssize_t epoll_read(int client_fd, char* buffer, size_t buffer_len);

/**
 * @brief Reads data from a socket, waiting until the requested number of bytes
 * have been read.
 *
 * @param client_fd  The file descriptor of the socket to read from.
 * @param buffer     A buffer to store the received data.
 * @param buffer_len The number of bytes to read.
 * @return The number of bytes read on success, -1 on error.  Will only return
 * less than buffer_len bytes if a signal interrupts the call or an error
 * occurs.
 */
 ...
 *
 *        This function registers a handler function to be called when the
 * specified signal is received. It also ignores the SIGPIPE signal, which can
 * occur when writing to a closed socket.
 *
 ...
 *
 *        This function listens for incoming connections on the specified server
 * socket and processes events using the provided I/O callback functions.
 *
 * @param server_fd The file descriptor of the server bound socket.
 * @param io_cb     A pointer to the io_callbacks structure containing the I/O
 * callback functions.
 * @return 0 on success, 1 on failure.
 */
...
/**
 * @brief Write message of length "length" to the connection buffer, growing it
 if necessary.
...
/**
 * @brief Parse request line, populating rh HTTP method, Path, http version and
 * start of headers.
 * @return true on success, false otherwise.
 */
