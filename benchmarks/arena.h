 ...
 *
 * Note: This only reserves address space - no physical memory is allocated until
 * you start allocating from the arena. Modern OSes can handle TB-scale reservations.
 *
 * Example:
 * ```c
 * // Reserve 1GB (uses ~0 MB of RAM initially)
 * Arena* arena = arena_create(1024 * 1024 * 1024);
 * if (!arena) {
...
static ARENA_INLINE bool arena_alloc_batch(Arena* arena, const size_t sizes[], size_t count, void* out_ptrs[]) {
    if (arena == nullptr || sizes == nullptr || out_ptrs == nullptr || count == 0) {
        return false;
    }
