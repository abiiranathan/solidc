...
#define DARRAY_ERROR(err, msg)                                                                     \
    fprintf(stderr, "%s:%d:darray error %d: %s\n", __FILE__, __LINE__, err, msg);
#else
#define DARRAY_ERROR(err, msg)
#endif
...
#define DARRAY_DEFINE(name, type)                                                                  \
    ...
    DARRAY_INLINE bool name##_push(name* arr, type value) {                                        \
        ...
        if (DARRAY_UNLIKELY(arr->count >= arr->capacity)) {                                        \
            size_t new_capacity =                                                                  \
                arr->capacity == 0 ? DARRAY_INIT_CAPACITY : arr->capacity * DARRAY_GROWTH_FACTOR;  \
    ...
    DARRAY_PURE DARRAY_INLINE const type* name##_const_data(const name* arr) {                     \
        return (const type*)(arr ? arr->items : NULL);                                             \
                                                                                                   ...
                                                                                                   \
    DARRAY_INLINE ptrdiff_t name##_find(const name* arr, const type* value,                        \
...
#define DARRAY_FOR_EACH_PTR(arr, var)                                                              \
    for (size_t _i = 0; _i < (arr)->count && ((var) = &(arr)->items[_i], 1); _i++)
