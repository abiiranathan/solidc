...
/**
 * Case-insensitive substring search (Windows implementation of POSIX strcasestr).
...
static inline int strcasecmp(const char* s1, const char* s2) {
    ...
    // If we've reached here, one or both strings ended
 ...
 * @return 0 if strings are equal up to n characters (case-insensitive),
 *         negative if s1 < s2, positive if s1 > s2.
 */
static inline int strncasecmp(const char* s1, const char* s2, size_t n) {
    ...
    for (size_t i = 0; i < n; i++) {
        const int c1 = tolower((unsigned char)s1[i]);
        const int c2 = tolower((unsigned char)s2[i]);

        if (c1 != c2) {
            return c1 - c2;
        }

        // If we hit null terminator in either string, we're done
        if (s1[i] == '\0') {
            return 0;
        }
    }
