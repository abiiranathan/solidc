    ...
    do {                                                                                           \
        ...
        if (_val < _min || _val > _max) {                                                          \
            printf("%s:%d [%s]: Value %ld is not in range [%ld, %ld].\n", __FILE__, __LINE__,      \
                   __func__, (long)_val, (long)_min, (long)_max);                                  \
            exit(1);                                                                               \
        }                                                                                          \
    } while (0)
...
#define ASSERT_FLOAT_EQ(a, b, epsilon)                                                             \
    do {                                                                                           \
        ...
        if ((_a - _b) > _eps || (_b - _a) > _eps) {                                                \
            printf(                                                                                \
                "%s:%d [%s]: Float assertion '%s == %s' failed (%.6f != %.6f, epsilon=%.6f).\n",   \
                __FILE__, __LINE__, __func__, #a, #b, _a, _b, _eps);                               \
            exit(1);                                                                               \
        }                                                                                          \
    } while (0)
...
#define FOR_EACH_ARRAY(item, array)                                                                \
    for (typeof((array)[0])*item = (array), *_end = (array) + ARRAY_SIZE(array); item < _end;      \
         ++item)
