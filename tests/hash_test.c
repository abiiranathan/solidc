...
uint32_t murmur_hash_wrapper(const void* key) {
    return solidc_murmur_hash(key, (uint32_t)strlen(key), 0);
}
...
int main() {
    ...
    test_hash_function("fnv1a", solidc_fnv1a_hash, "hello", 1335831723);
    test_hash_function("elf", solidc_elf_hash, "hello", 7258927);
    test_hash_function("djb2a", solidc_djb2a_hash, "hello", 178056679);
    test_hash_function_with_length("crc32", solidc_crc32_hash, "hello", 907060870);
    test_hash_function("murmur", murmur_hash_wrapper, "kinkajou", 3067714808);
